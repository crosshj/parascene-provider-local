...
const WORKFLOW_TEMPLATE = JSON.parse(
  fs.readFileSync(path.join(__dirname, "pony-checkpoint.json"), "utf8"),
);
...
function PonyWorkflow(overrides = {}) {
  const workflow = cloneBaseWorkflow();
  if (overrides.modelFile) {
    const group = overrides.comfyCheckpointGroup || "pony";

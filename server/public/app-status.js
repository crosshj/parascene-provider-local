...
function extractPromptId(queueEntry) {
  ...
  if (
    typeof queueEntry === "object" &&
    typeof queueEntry.prompt_id === "string"
  ) {
...
async function refresh() {
  try {
    ...
    set(
      "comfyStatsHttp",
      comfy.system_stats_http_status != null
        ? comfy.system_stats_http_status
        : "—",
    );
    set(
      "comfyQueueHttp",
      comfy.queue_http_status != null ? comfy.queue_http_status : "—",
    );
    ...
    if (
      typeof system.ram_free === "number" &&
      typeof system.ram_total === "number"
    ) {
      set(
        "comfyRam",
        `${fmtBytes(system.ram_free)} / ${fmtBytes(system.ram_total)}`,
      );
    } else {
      set("comfyRam", "—");
    }
    ...
    if (jobs.byModel && typeof jobs.byModel === "object") {
      ...
      for (const [key, val] of Object.entries(jobs.byModel)) {
        const pending =
          val && typeof val.pending === "number" ? val.pending : 0;

...
function SdxlWorkflow(overrides = {}) {
  ...
  workflow["31"].inputs.steps = toPositiveInt(overrides.steps, 32);
  workflow["31"].inputs.cfg = toNumber(overrides.cfg, 5.5);
  workflow["27"].inputs.width = toPositiveInt(overrides.width, 1024);
  workflow["27"].inputs.height = toPositiveInt(overrides.height, 1024);
  return workflow;
}

module.exports = SdxlWorkflow;

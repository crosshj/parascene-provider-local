...
## 4. Phase 1 scope (Bootable Windows Service)
...
| Install script | `service/scripts/install.js` | Create runtime/logs; generate `service.generated.xml` for WinSW (working dir = release root). |
...
- **Phase 2:** Worker manager + dummy worker (heartbeat, restart rules).  
